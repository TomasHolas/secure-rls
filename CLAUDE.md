...
2. **Defense-in-depth RLS** (ADR 0002 as amended). Identity layer (JWT claim),
   ...
   plus query timeouts, a result-row cap with truncation signaling and a
   character cap on the model's copy of one result (ADR 0007 as amended),
...
- `[x]` **M2 — Agent.** `agent.py`: explicit LangGraph graph on Ollama with
  ...
  (ADR 0011 as amended), and its sibling on what ONE tool result may contribute,
  cutting the model's copy at a line boundary and reporting `withheld` while the
  reader keeps every row (ADR 0007 as amended); the grounding
...
- `[x]` **M3 — REST API + auth.** `app.py` (thin handlers: `/login`, `/chat` as
  an SSE stream of typed trace events over a turn that runs independently of it
  (`inflight.py`, ADR 0012 as amended), `/conversations` JWT-scoped CRUD,
...
- `[x]` **M4 — Frontend.** React SPA on the KB design system: login, streaming
  ...
  Four shell tabs (ADR 0014 as rewritten and amended): **Chat**, **Records** (the
  whole dataset — all 1000 rows, all three tenants — filtered, sorted and paged
  server-side with `tenant_id` a filter like any other, and the executed
  statement shown and labelled as unscoped), **Notes** (the whole corpus,
  poisoned badges across every tenant, plus a search that runs the agent's own
  SCOPED retrieval path and shows the distances) and **Audit** (the audit log
  itself — every statement the data path ran, its verdict and the statement that
  executed, newest first, all tenants, no filters). The tabs are the control
...
| A turn that outlives its stream (the worker, the reader's window, one turn per thread) | `apps/backend/inflight.py` — `InFlightTurns.start` runs a turn on its own thread and hands back a bounded window onto its frames, so the recorder and the checkpointer are fed by the turn and never by whoever is watching (ADR 0012 as amended). Forwarding is what stops when a reader leaves or stops draining (`api.turn_queue_frames`); the turn never does. It also owns the one-turn-per-thread claim: `start` raises `TurnBusy` (the `/chat` 409) and `running` is what `GET /conversations/{id}` reports as `in_flight`. It bounds nothing — the turn's deadline and tool-round cap are the graph's (ADR 0011) |
...
| Records/Notes browsing (allowlisted filters, sorts, paging, filter options, the poison manifest) | `apps/backend/browse.py` — fixed templates with bound filter values. The LISTINGS run through `db.execute_unscoped` and show the whole dataset, with `tenant_id` a bound filter and a sortable column like `department` (ADR 0014 as rewritten); the notes SEARCH is the agent's own path for whoever is asking (ADR 0014 section 6 as amended): scoped for a tenant identity, the partition-less retrieval for an all-tenant one, with `annotate_note_hits` joining each hit's tenant, department and score off its row through the SAME scope - `db.execute_scoped`, or `db.execute_unscoped` only when the verified token grants every tenant - so a retrieval claim is checkable and a cross-tenant hit is described rather than blanked. The scope comes from `Identity`; no query parameter can widen it. Sort, direction and an options column are allowlisted words, never bound values. `filter_options` serves the tenant and department pickers from the same read, so no count describes a set nobody asked for. `ignored_params` reports the parameters a listing did not read — names only — so a discarded parameter is stated rather than swallowed |
...
| Frontend chat stream (SSE frames -> typed trace events -> one turn's state) | `apps/frontend/src/lib/sse.ts` + `lib/trace.ts`; rendered by `views/ChatView.tsx` over the `components/chat/` bricks. `lib/trace.ts` is the ONE fold: `replayTurns` runs a reopened thread's stored events through `applyEvent` itself, so a past turn is the same `Turn` object — and therefore the same bricks — as a live one. The only differences a replayed turn carries are the two it cannot have: no token-by-token arrival, and no measured thinking span (that clock is this browser's). `replayTurns` also takes the thread's `in_flight`: a thread reopened while its turn still runs on the server replays that last question as pending instead of as a turn whose trace was lost (ADR 0012 as amended) |
| Frontend Records / Notes tabs | `apps/frontend/src/views/RecordsView.tsx`, `views/NotesView.tsx` — the whole dataset, with `tenant_id` a filter like any other and every total naming what it counts ("1,000 rows · all tenants", "450 · tenant acme"); filters, sorts and pages are query parameters, never in-browser reordering, and every filter applies itself - chips and selects on the change, a typed box once the reader stops typing (issue #152), with no Apply button on either tab; the tab strip is the `layout/Tabs` brick in the shell header and a visited tab stays mounted; the tenant filter is a `forms/ChipRow` (no counts on the chips) and the executed statement sits behind a closed `Disclosure` under a caption stating that the listing is unscoped by design. The reader-facing parameter probe was removed on owner review (issue #139) — `browse.ignored_params` still reports every unread parameter in the response, checkable with curl or a network tab (ADR 0014 as rewritten and amended) |
| Frontend Audit tab | `apps/frontend/src/views/AuditView.tsx` — the audit log under the store's own column names (`ts`, `tenant`, `generated_sql`, `verdict`, `error_kind`, `executed_sql`, `rowcount`), newest first, verdict as a `Pill` (neutral ran, `danger` refused, `warn` broke) and each statement one ellipsised line with the full text on `title`. No filters on purpose — a pager and a reload, because a mounted tab does not refetch while the reader is elsewhere; a tenant chip row is the owner's to ask for (ADR 0014 section 12) |
| Frontend conversation state (thread list, which thread is open, its replay) | `apps/frontend/src/lib/conversations.ts` — the one owner the rail (`views/ConversationsSidebar.tsx`) and the chat view share; `replay` is the open thread's past turns, already folded; `rename` sends the reader's own title through the same PATCH `titleThread` uses and adopts the row that comes back, so which name wins stays the server's rule (the `renamed` flag) and never a second one here. `select` answers with the thread it opened, or null when the registry refused it, which is what lets the shell clean a URL naming a thread that is gone |
| Where the reader is (which tab, which thread) | `apps/frontend/src/lib/location.ts` — the ONE owner of the URL hash: `#/chat/<thread_id>`, `#/chat`, `#/records`, `#/notes`, `#/audit` and nothing else ever, so a reload lands back where the reader was (issue #135). Parse, subscribe (a `hashchange`/`popstate` store like `auth.ts`, read through `useSyncExternalStore`), push for somewhere to come back to, replace for a restatement, clear on logout. `App.tsx` reconciles it with `lib/conversations.ts` through that store's own `select`, so a restored thread renders through the path a click takes; a thread the registry will not open lands on the draft with its existing message and the hash is cleaned — deleted and foreign stay indistinguishable (ADR 0012). The hash carries no token, no query and no view's filters |
| The conversation rail itself (collapse, inline search, hover glide, the inline rename) | `apps/frontend/src/views/ConversationsSidebar.tsx` over the `components/layout/Sidebar` collapse mechanism plus the `InlineSearch` and `GlideList` bricks; every metric and duration is a custom property in the rail's block in `styles/app.css` (issue #114, `docs/ui-pattern-review.md`). The aside clips (`overflow: clip`) while the column inside it stays at the expanded width — that is what keeps its icons from moving — so a slot with clipped controls reads `useSidebarCollapsed()` and takes them out of the Tab order rather than being unmounted. The pencil beside the delete control turns a row's title into a box that takes the title's own typography and adds no box of its own, so the row keeps its height: Enter and blur save through `store.rename`, Escape and an emptied box cancel, and a blank title is never sent because the server refuses it |
| When a typed filter applies itself | `apps/frontend/src/lib/debounce.ts` — the one place the filter debounce interval lives (`FILTER_DEBOUNCE_MS`), with the scheduler, its cancel, and the still-typing flag the Records tab holds a half-typed value's refusal back with, so no banner flashes between two keystrokes (ADR 0014 as amended) |
...
| The six pipeline steps themselves | `apps/frontend/src/components/pipelineSteps.ts` — the ONE list `PipelineCanvas` (the map) and `chat/PipelineStrip` (the journey) both read, so two drawings of one enforcement path cannot come to disagree about its labels or its order |
| Where one statement actually got (the per-query strip) | `apps/frontend/src/components/chat/PipelineStrip.tsx` — the canvas's six steps compacted to a line of `Pill` chips under an executed statement: a result lights the whole path (a payload carrying `executed_sql` proves it, since every layer raises rather than returns), a refusal lights only what provably ran and marks the layer that stopped it, off the `layer`/`kind` pair `agent.py` puts on the wire. Never a prediction — no strip while a call is in flight — and a layer identifier it cannot place renders no strip rather than a wrong one (`docs/ui-pattern-review.md`) |
...
| Assignment-facing docs | `README.md` is the short form only — one-line security-layer summary, a two-line run pointer (the full quickstart lives in `docs/quickstart.md`), creds, the eval headline table, the deliverables map, challenge-decision-outcome bullets, one-line limitations, time spent; it links every claim to its owner and stays near ~130 lines (issues #133, #150, the owner's final cut). The depth has exactly one home each: `docs/architecture.md` (layers, tools, components, the browse control group, the compliance map), `docs/api.md` (routes, the SSE frames and their invariants, sessions), `docs/development-process.md` (agentic method, tests - the ONE owner of test totals, the eval harness and gate, CI/CD), `docs/challenges.md` (per-wave challenges, known limitations). A fact belongs in one of those and is summarized, never duplicated, in the README |
