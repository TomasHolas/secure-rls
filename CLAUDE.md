...
| The chat's empty state (the query pipeline as a flowchart) | `apps/frontend/src/components/PipelineCanvas.tsx` — six cards on a dotted canvas joined by measured beziers, draggable, a selected card lighting its connectors; rendered by `views/ChatView.tsx` only while a thread has no turn, and the copy is one line per layer from the README's security table so screen and docs cannot drift. Every metric is a custom property in `app.css`; the pattern is beautifului.dev's Flowchart on our tokens (`docs/ui-pattern-review.md`) |
