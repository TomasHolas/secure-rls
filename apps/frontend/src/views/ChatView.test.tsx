...
describe("the chat view", () => {
  ...
  it("marks the tenant scoping inside the statement that ran", async () => {
    ...
    expect(marked(view.container)).toContain("tenant_id");
    expect(screen.queryByText("generated by the model")).toBeNull();
  });

  it("shows both statements whole when the reader asks for the pair", async () => {
    await renderReady();
    ask();
    fireEvent.click(await screen.findByRole("button", { name: "show both" }));
